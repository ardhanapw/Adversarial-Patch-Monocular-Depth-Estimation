import os
from models.monovit import networks
import torch
...
class MonoViT(ModelMDE):
    def __init__(self, model_path, device=None, **kwargs):
        self.encoder = networks.mpvit_small() #ResnetEncoder(18, False)
        self.encoder.num_ch_enc = [64,128,216,288,288]
        self.depth_decoder = networks.DepthDecoder() #DepthDecoder(self.encoder.num_ch_enc, scales=range(4))
        self.device = device
        #self.model, self.input_height, self.input_width = self.load(model_path, device)
        super(MonoViT, self).__init__(device=self.device, model_path=model_path, **kwargs)
