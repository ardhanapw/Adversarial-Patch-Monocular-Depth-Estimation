import os
from models.lite_mono import networks
import torch
...
class LiteMono(ModelMDE):
    def __init__(self, model_path, device=None, **kwargs):
        self.encoder = networks.LiteMono(model='lite-mono-8m', height=192, width=640) #ResnetEncoder(18, False)
        ...
        super(LiteMono, self).__init__(device=self.device, model_path=model_path, **kwargs)
    ...
    def load(
        ...
        filtered_dict_dec ={k: v for k, v in decoder_dict.items() if k in depth_decoder.state_dict()}
        depth_decoder.load_state_dict(filtered_dict_dec)
        depth_decoder.to(device)
        depth_decoder.eval()
        #print(filtered_dict_dec)
        #print(filtered_dict_enc)
